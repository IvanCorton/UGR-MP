...
void mostrarPalabras(string **indice, string inicial){
	...
	//Fallo: usar char letra = toupper(inicial);
//	 usar string* ptrIni = indice[letra - 'A']
//	 usar string* ptrFin = indice [(letra+1)-'A']
...
int cuentaPalabras(string **indice, string inicial){
	...
	//char letra = toupper(inicial);
	//ptrIni y ptrFin. return diferencia.
...
bool existe(string ** indice, string palabra){
	...
	//Al igual usar ptrIni y prtFin para ver en un rango
	//si esta una palabra concreta
	// existe = *ptrIni == pal;
...
void agregaPalabra(string ** & indice, string * & datos, int &datosUtiles, string pal){
	...
	//FALLO: Comprobacion inicial de "si cabe"
	if(!existe(indice, pal)){

		if(pal[0]!='Z'){


			//FALLO: Usar funcion posPalabra
			for(auto i=0; i<numAbc-1; i++){
				if(pal[0] == (*indice[i])[0]){
					posicionIndices = i+1;
				}
			...
			//FALLO: Usar funcion hacerHueco
			for(auto i=datosUtiles; i>=posicionDatos; i--){
				datos[i] = datos[i-1];
			}

		//FALLO: Basta con adelantar los indices no tener que ir mirando uno a uno.
			for(auto i=0; i<numAbc-1; i++){
...
void borraPalabra(string ** & indice, string * & datos, int &datosUtiles, string pal){
	...
	if(existe(indice, pal)){
		...
		//FALLO: Utilizar funcion aux posPalabra
		for(auto i=0;i<datosUtiles-1; i++){
			if(datos[i]==pal){
				posicionDatos = i;
			}
		...
		//FALLO: Utilizar funcion borrarHueco
