...
void test3(){
    cout << "\nTest 3: Constructor por defecto, de copia, con parámetros y asignación" << endl;
...
void ordenar(int numPol){
    
		PoliReg *v;

	    v = new PoliReg[numPol];

	    for(auto i=0; i<numPol;i++){
		v[i] = PoliReg(5+rand()%6, Punto2D(), 20 + rand()%130);
	    }

		PoliReg temporal;

	    for (int i = 0;i < numPol; i++){
		for (int j = 0; j< numPol-1; j++){
			if (v[j] < v[j+1]){
				temporal = v[j]; 
				v[j] = v[j+1]; 
				v[j+1] = temporal;
